...
public class DevConsole
{
	...
	{
		FileAccess val = FileAccess.Open(_historyFilePath, (ModeFlags)1);
		try
		{
			if (val != null)
			{
				while (val.GetPosition() < val.GetLength())
				{
					history.Add(val.GetLine());
				}
			}
		}
		finally
	...
	{
		FileAccess val = FileAccess.Open(_historyFilePath, (ModeFlags)2);
		try
		{
			if (val == null)
			{
