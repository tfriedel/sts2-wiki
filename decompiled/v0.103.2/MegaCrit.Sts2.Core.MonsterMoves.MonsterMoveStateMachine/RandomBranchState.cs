...
public class RandomBranchState : MonsterState
{
	...
	public void AddBranch(MonsterState state, int cooldown, MoveRepeatType repeatType, Func<float> weight)
	{
		...
		StateWeight stateWeight = default(StateWeight);
		stateWeight.repeatType = repeatType;
		stateWeight.stateId = state.Id;
		stateWeight.weightLambda = weight;
		stateWeight.cooldown = cooldown;
		StateWeight item = stateWeight;
		States.Add(item);
	}
	...
	{
		StateWeight stateWeight = default(StateWeight);
		stateWeight.maxTimes = maxRepeats;
		stateWeight.repeatType = MoveRepeatType.CanRepeatXTimes;
		stateWeight.stateId = state.Id;
		stateWeight.weightLambda = weight;
		stateWeight.cooldown = cooldown;
		StateWeight item = stateWeight;
	...
	{
		Creature owner2 = owner;
		float max = States.Sum((StateWeight x) => GetStateWeight(x, owner2));
		float num = rng.NextFloat(max);
		foreach (StateWeight state in States)
		{
			num -= GetStateWeight(state, owner2);
			if (num <= 0f)
			{
