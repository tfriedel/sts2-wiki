...
public class EventSplitVoteAnimation
{
	...
	public async Task TryPlay(NEventOptionButton chosenButton)
	{
		...
		Tween val = ((Node)_eventLayout).CreateTween();
		val.TweenMethod(Callable.From<float>((Action<float>)TickSplitVoteAnimation), Variant.op_Implicit(0f), Variant.op_Implicit(1f), 1.2000000476837158).SetTrans((TransitionType)1).SetEase((EaseType)1);
		val.TweenInterval((double)num);
		await ((GodotObject)_eventLayout).ToSignal((GodotObject)(object)val, SignalName.Finished);
