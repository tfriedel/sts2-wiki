...
public record CardCreationOptions
{
	...
	public IEnumerable<CardModel> GetPossibleCards(Player player)
	{
		...
		IEnumerable<CardModel> enumerable = ((CardPools.Count <= 0) ? CustomCardPool : (from c in CardPools.SelectMany((CardPoolModel p) => p.GetUnlockedCards(player2.UnlockState, player2.RunState.CardMultiplayerConstraint))
	...
	public CardRarity? TryGetSingleRarityInPool()
	{
		...
		{
			CardModel first2 = CustomCardPool.FirstOrDefault();
			if (first2 != null && CustomCardPool.All((CardModel c) => c.Rarity == first2.Rarity))
			{
