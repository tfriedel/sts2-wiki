...
public class Watchdog
{
	...
	public static string DumpState()
	{
		...
		try
		{
			...
			Node instance = (Node)(object)NGame.Instance;
			if (instance != null)
			{
				...
				if (nodeOrNull != null)
				{
					foreach (Node child in nodeOrNull.GetChildren(false))
					{
						...
						string value = ((val != null) ? (((CanvasItem)val).Visible ? "visible" : "hidden") : "");
						StringBuilder stringBuilder2 = stringBuilder;
						...
						handler.AppendLiteral("  - ");
						handler.AppendFormatted<StringName>(child.Name);
						handler.AppendLiteral(" (");
						handler.AppendFormatted(((object)child).GetType().Name);
						handler.AppendLiteral(") [");
