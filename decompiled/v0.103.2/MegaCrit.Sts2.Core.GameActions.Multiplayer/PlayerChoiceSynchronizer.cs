...
public class PlayerChoiceSynchronizer : IDisposable
{
	...
	public void SyncLocalChoice(Player player, uint choiceId, PlayerChoiceResult result)
	{
		...
		PlayerChoiceMessage playerChoiceMessage = default(PlayerChoiceMessage);
		playerChoiceMessage.result = result.ToNetData();
		playerChoiceMessage.choiceId = choiceId;
		PlayerChoiceMessage message = playerChoiceMessage;
	...
	public async Task<PlayerChoiceResult> WaitForRemoteChoice(Player player, uint choiceId)
	{
		...
		if (!ValidateChoiceId(player2, choiceId))
		{
			throw new InvalidOperationException($"Tried to wait for remote choice with ID {choiceId} for player {player2.NetId}, but player's choice ID is {GetChoiceId(player2)}!");
		}
		int num = _receivedChoices.FindIndex((ReceivedChoice c) => c.choiceId == choiceId && c.senderId == player2.NetId);
		ReceivedChoice item;
		if (num >= 0)
		{
			_logger.Debug($"Was going to wait for remote choice {choiceId} for player {player2.NetId} but we've already received it");
			item = _receivedChoices[num];
			_receivedChoices.RemoveAt(num);
		}
		else
		{
			ReceivedChoice receivedChoice = default(ReceivedChoice);
			receivedChoice.choiceId = choiceId;
			receivedChoice.senderId = player2.NetId;
			receivedChoice.completionSource = new TaskCompletionSource<NetPlayerChoiceResult>();
			item = receivedChoice;
			_logger.Debug($"Awaiting remote choice {choiceId} for player {player2.NetId}");
			_receivedChoices.Add(item);
		}
		NetPlayerChoiceResult netData = await item.completionSource.Task;
		PlayerChoiceResult playerChoiceResult = PlayerChoiceResult.FromNetData(player2, _players, netData);
		_logger.Debug($"Finished waiting for remote choice {choiceId} for player {player2.NetId}: {playerChoiceResult}");
	...
	{
		Player player2 = player;
		this.PlayerChoiceReceived?.Invoke(player2, choiceId, result);
		int num = _receivedChoices.FindIndex((ReceivedChoice c) => c.choiceId == choiceId && c.senderId == player2.NetId);
		ReceivedChoice item;
		if (num >= 0)
		{
