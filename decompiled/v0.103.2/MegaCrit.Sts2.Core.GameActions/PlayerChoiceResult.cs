...
public class PlayerChoiceResult
{
	...
	public static PlayerChoiceResult FromNetData(Player sender, IPlayerCollection players, NetPlayerChoiceResult netData)
	{
		...
		case PlayerChoiceType.DeckCard:
			playerChoiceResult._deckCards = netData.deckCards.Select((NetDeckCard c) => c.ToCardModel(sender2)).ToList();
			break;
	...
	{
		NetPlayerChoiceResult result = default(NetPlayerChoiceResult);
		result.type = ChoiceType;
		result.combatCards = _combatCards?.Select(NetCombatCard.FromModel).ToList();
		result.deckCards = _deckCards?.Select(NetDeckCard.FromModel).ToList();
		result.canonicalCards = _canonicalCards?.ToList();
		result.mutableCards = _mutableCards?.Select((CardModel c) => c.ToSerializable()).ToList();
		result.mutableCardOwner = _mutableCards?.FirstOrDefault()?.Owner.NetId;
		result.playerId = _playerId;
		result.indexes = _indexes;
		return result;
	}
