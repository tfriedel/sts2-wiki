...
public class OneOffSynchronizer : IDisposable
{
	...
	private async Task<int> DoTreasureRoomRewards(Player player)
	{
		...
		double gold2 = player.PlayerRng.Rewards.NextInt(42, 53);
		if (AscensionHelper.HasAscension(AscensionLevel.Poverty))
		{
			gold2 *= AscensionHelper.PovertyAscensionGoldMultiplier;
		}
		await PlayerCmd.GainGold((int)gold2, player);
		double num = gold2;
		gold2 = num + (double)(await TryHandleSpoilsMap(player));
		return (int)gold2;
	}
