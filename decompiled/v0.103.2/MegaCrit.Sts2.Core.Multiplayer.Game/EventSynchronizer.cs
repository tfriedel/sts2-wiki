...
public class EventSynchronizer : IDisposable
{
	...
	private void ChooseSharedEventOption()
	{
		...
		SharedEventOptionChosenMessage sharedEventOptionChosenMessage = default(SharedEventOptionChosenMessage);
		sharedEventOptionChosenMessage.optionIndex = value;
		sharedEventOptionChosenMessage.pageIndex = _pageIndex;
		sharedEventOptionChosenMessage.location = _messageBuffer.CurrentLocation;
		SharedEventOptionChosenMessage message = sharedEventOptionChosenMessage;
	...
	public void ChooseLocalOption(int index)
	{
		if (IsShared)
		{
			...
			VotedForSharedEventOptionMessage message = default(VotedForSharedEventOptionMessage);
			message.optionIndex = (uint)index;
			message.pageIndex = _pageIndex;
		...
		else
		{
			...
			OptionIndexChosenMessage message2 = default(OptionIndexChosenMessage);
			message2.type = OptionIndexType.Event;
			message2.optionIndex = (uint)index;
			message2.location = _messageBuffer.CurrentLocation;
	...
	private void SaveEventOptionToHistory(Player player, EventOption option)
	{
		...
		EventOptionHistoryEntry eventOptionHistoryEntry = default(EventOptionHistoryEntry);
		eventOptionHistoryEntry.Title = option.HistoryName;
		eventOptionHistoryEntry.Variables = new Dictionary<string, object>();
		EventOptionHistoryEntry item = eventOptionHistoryEntry;
