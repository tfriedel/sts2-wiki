...
public static class ModManager
{
	...
	[CompilerGenerated]
	private sealed class _003CGetModdedLocTables_003Ed__29 : IEnumerable<string>, IEnumerable, IEnumerator<string>, IEnumerator, IDisposable
	{
		...
		[DebuggerHidden]
		public _003CGetModdedLocTables_003Ed__29(int _003C_003E1__state)
		{
		...
		private bool MoveNext()
		{
			try
			{
				...
				while (_003C_003E7__wrap1.MoveNext())
				{
					...
					if (current.state == ModLoadState.Loaded)
					{
						string text = $"res://{current.manifest.id}/localization/{language}/{file}";
		...
		{
			_003CGetModdedLocTables_003Ed__29 _003CGetModdedLocTables_003Ed__;
			...
			{
				_003CGetModdedLocTables_003Ed__ = new _003CGetModdedLocTables_003Ed__29(0);
			}
	...
	private static void SortModList(List<SettingsSaveMod> manualOrdering)
	{
		...
		{
			List<int> list3 = HasCircularDependenciesRecursive(j, seenMods, currentChain, list);
			if (list3 == null)
			{
				continue;
			}
		...
		for (int k = 0; k < manualOrdering.Count; k++)
		{
			dictionary2[manualOrdering[k].Id] = k;
		}
		for (int l = 0; l < _mods.Count; l++)
		{
			Mod mod3 = _mods[l];
			if (list[l] == 0)
			{
		...
		{
			for (int n = 0; n < manualOrdering.Count; n++)
			{
				if (manualOrdering[n].Id != list4[n].manifest?.id)
				{
					flag = true;
					break;
				}
		...
		if (flag)
		{
			...
			{
				Log.Info($"  {num3}: {list4[num3].manifest?.name} ({list4[num3].manifest?.id})");
			}
		}
		_mods = list4;
	}
	...
	private static Mod? ReadModManifest(string filename, ModSource source)
	{
		...
		try
		{
			using Stream utf8Json = _fileIo.OpenStream(filename, (ModeFlags)1);
	...
	[IteratorStateMachine(typeof(_003CGetModdedLocTables_003Ed__29))]
	public static IEnumerable<string> GetModdedLocTables(string language, string file)
	{
		//yield-return decompiler failed: Unexpected instruction in Iterator.Dispose()
		return new _003CGetModdedLocTables_003Ed__29(-2)
		{
	...
	public static bool HasHarmonyPatches()
	{
		try
		{
			bool valueOrDefault = _hasHarmonyPatches.GetValueOrDefault();
