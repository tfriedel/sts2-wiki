...
public static class MultiplayerDebugUtil
{
	...
	{
		IReadOnlyList<byte> source2 = source;
		return from e in source2.Select((byte _, int index) => source2.Skip(size * index).Take(size).ToArray()).TakeWhile((byte[] bucket) => bucket.Length != 0)
	...
	public static string FormatAsHex(ReadOnlySpan<byte> data, int lineWidth = 16, int byteWidth = 1)
	{
		...
		for (int i = 0; i < data.Length; i += array.Length)
		{
			...
			string @string = Encoding.ASCII.GetString(ReplaceControlCharactersWithDots(array));
