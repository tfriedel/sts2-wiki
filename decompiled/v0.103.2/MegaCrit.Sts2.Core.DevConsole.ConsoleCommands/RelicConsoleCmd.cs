...
public class RelicConsoleCmd : AbstractConsoleCmd
{
	...
	{
		string id2 = id;
		id2 = id2.ToUpperInvariant();
		List<RelicModel> source = ModelDb.AllRelics.Where((RelicModel r) => r.Id.Entry.Contains(id2)).ToList();
		RelicModel relicModel = source.FirstOrDefault((RelicModel r) => r.Id.Entry == id2);
		if (relicModel != null)
		{
			return relicModel;
		}
		RelicModel relicModel2 = source.FirstOrDefault((RelicModel r) => r.Id.Entry.StartsWith(id2));
		if (relicModel2 != null)
		{
			return relicModel2;
		}
