...
public abstract class AbstractConsoleCmd
{
	...
	{
		Func<string, string, bool> matchPredicate2 = matchPredicate;
		string partialArg2 = partialArg;
		List<string> list = candidates.ToList();
		if (matchPredicate2 == null)
		{
			matchPredicate2 = (string candidate, string partial) => candidate.StartsWith(partial, StringComparison.OrdinalIgnoreCase);
		}
		List<string> list2 = ((!string.IsNullOrWhiteSpace(partialArg2)) ? list.Where((string c) => matchPredicate2(c, partialArg2)).ToList() : list);
