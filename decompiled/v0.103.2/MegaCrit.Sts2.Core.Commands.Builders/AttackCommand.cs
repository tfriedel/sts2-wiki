	...
	public async Task<AttackCommand> Execute(PlayerChoiceContext? choiceContext)
	{
		...
		for (int i = 0; (decimal)i < attackCount; i++)
		{
			...
			if (_playOnEveryHit || i == 0)
			{
				...
				{
					((Node)(object)NCombatRoom.Instance?.CombatVfxContainer).AddChildSafely((Node?)(object)customAttackerVfxNode());
				}
			...
			foreach (Func<Creature, Node2D> customHitVfxNode in _customHitVfxNodes)
			{
				...
				{
					((Node)(object)NCombatRoom.Instance?.CombatVfxContainer).AddChildSafely((Node?)(object)customHitVfxNode(singleTarget));
					continue;
				}
				foreach (Creature item in validTargets)
				{
					((Node)(object)NCombatRoom.Instance?.CombatVfxContainer).AddChildSafely((Node?)(object)customHitVfxNode(item));
				}
			}
			if (HitVfx != null)
			{
