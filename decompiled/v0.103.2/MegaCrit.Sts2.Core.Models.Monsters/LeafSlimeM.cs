	...
	private async Task StickyShotMove(IReadOnlyList<Creature> targets)
	{
		if (TestMode.IsOff)
		{
			...
			foreach (Creature target in targets)
			{
				...
				if (creatureNode != null && (!val.HasValue || val.Value.X > ((Control)creatureNode).GlobalPosition.X))
				{
					val = creatureNode.VfxSpawnPosition;
				}
			}
			Node2D val2 = NCombatRoom.Instance.GetCreatureNode(base.Creature)?.GetSpecialNode<Node2D>("Visuals/SpitTarget");
			if (val2 != null)
			{
				val2.GlobalPosition = val.Value;
			}
		}
		SfxCmd.Play(CastSfx);
