...
public sealed class KnowledgeDemon : MonsterModel
{
	...
	private async Task ChooseCurse(Creature target)
	{
		...
		{
			CardModel cardModel2 = base.CombatState.CreateCard((CardModel)c, target2.Player);
		...
		CardModel cardModel = await CardSelectCmd.FromChooseACardScreen(new BlockingPlayerChoiceContext(), cards, target2.Player);
		if (cardModel != null)
		{
