	...
	private async Task BoomerangMove(IReadOnlyList<Creature> targets)
	{
		if (TestMode.IsOff)
		{
			...
			foreach (Creature target in targets)
			{
				...
				if (!val.HasValue || val.Value.X > ((Control)creatureNode).GlobalPosition.X)
				{
					val = ((Control)creatureNode).GlobalPosition;
				}
			}
			...
			if (specialNode != null)
			{
				specialNode.Position = Vector2.Left * (((Control)creatureNode2).GlobalPosition.X - val.Value.X) / creatureNode2.Body.Scale;
			}
		}
