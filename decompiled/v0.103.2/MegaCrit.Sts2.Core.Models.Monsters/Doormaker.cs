...
public sealed class Doormaker : MonsterModel
{
	...
	private void UpdateVisual(string path)
	{
		...
		if (nCreature != null)
		{
			...
			Tween val = ((Node)nCreature).CreateTween();
			val.TweenProperty((GodotObject)(object)nCreature.Visuals.GetCurrentBody(), NodePath.op_Implicit("scale"), Variant.op_Implicit(scale), 1.2000000476837158).From(Variant.op_Implicit(scale * 0.5f)).SetEase((EaseType)1)
				.SetTrans((TransitionType)1);
			val.Parallel().TweenProperty((GodotObject)(object)nCreature.Visuals.GetCurrentBody(), NodePath.op_Implicit("modulate"), Variant.op_Implicit(Colors.White), 0.5).From(Variant.op_Implicit(Colors.Black));
		}
	}
