...
public sealed class TorchHeadAmalgam : MonsterModel
{
	...
	public override void OnDieToDoom()
	{
		...
		if (creatureNode != null)
		{
			...
			if (specialNode != null)
			{
				((CanvasItem)specialNode).SetVisible(false);
			}
			...
			{
				((CanvasItem)specialNode2).SetVisible(false);
			}
			...
			{
				((CanvasItem)specialNode3).SetVisible(false);
			}
		}
	}
	...
	private async Task SoulBeamMove(IReadOnlyList<Creature> targets)
	{
		...
		if (nCreature != null)
		{
			...
			if (specialNode != null)
			{
				...
				specialNode.Position += Vector2.Left * (((Control)creatureNode).GlobalPosition.X - ((Control)nCreature).GlobalPosition.X + 3000f);
			}
		}
