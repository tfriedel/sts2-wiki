...
public sealed class SlumberingBeetle : MonsterModel
{
	...
	public override async Task AfterAddedToRoom()
	{
		...
		Marker2D val = NCombatRoom.Instance.GetCreatureNode(base.Creature)?.GetSpecialNode<Marker2D>("%SleepVfxPos");
		if (val != null)
		{
			SleepingVfx = NSleepingVfx.Create(((Node2D)val).GlobalPosition);
			((Node)(object)val).AddChildSafely((Node?)(object)SleepingVfx);
	...
	private async Task RolloutMove(IReadOnlyList<Creature> targets)
	{
		...
		if (nCreature != null)
		{
			...
			if (specialNode != null)
			{
				specialNode.Position = Vector2.Left * (((Control)nCreature).GlobalPosition.X - ((Control)creatureNode).GlobalPosition.X);
			}
		}
