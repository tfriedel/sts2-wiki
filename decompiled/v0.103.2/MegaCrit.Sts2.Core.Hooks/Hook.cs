...
public static class Hook
{
	...
	public static async Task BeforePlayPhaseStart(HookPlayerChoiceContext playerChoiceContext, Task setupPlayerTurnTask, CombatState combatState, Player player)
	{
		...
		{
			playerChoiceContext.PushModel(model2);
			await model2.BeforePlayPhaseStart(playerChoiceContext, player);
			model2.InvokeExecutionFinished();
			playerChoiceContext.PopModel(model2);
		}
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			playerChoiceContext.PushModel(model2);
			await model2.BeforePlayPhaseStartLate(playerChoiceContext, player);
			model2.InvokeExecutionFinished();
			playerChoiceContext.PopModel(model2);
		}
	}
	...
	public static async Task AfterSideTurnStart(CombatState combatState, CombatSide side)
	{
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			await model2.AfterSideTurnStart(side, combatState);
			model2.InvokeExecutionFinished();
		}
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			await model2.AfterSideTurnStartLate(side, combatState);
			model2.InvokeExecutionFinished();
		}
	}
