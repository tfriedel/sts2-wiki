...
public class LocTable
{
	...
	{
		string keyPrefix2 = keyPrefix;
		HashSet<string> hashSet = new HashSet<string>(_translations.Keys.Where((string k) => k.StartsWith(keyPrefix2)));
		if (_fallback != null)
		{
			foreach (string item in _fallback.Keys.Where((string k) => k.StartsWith(keyPrefix2)))
			{
				hashSet.Add(item);
			}
		}
