				...
				if (result != null)
				{
					...
					if (cardPileAddResult.success)
					{
						...
						if (cardHolder != null)
						{
							...
							((Node)(object)cardHolder).QueueFreeSafely();
							Vector2 targetPosition = PileType.Deck.GetTargetPosition(cardNode);
							NRun.Instance.GlobalUi.TopBar.TrailContainer.AddChildSafely((Node?)(object)NCardFlyVfx.Create(cardNode, targetPosition, isAddingToPile: true, result.Owner.Character.TrailPath));
						}
