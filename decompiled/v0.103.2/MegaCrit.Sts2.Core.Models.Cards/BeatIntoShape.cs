...
public sealed class BeatIntoShape : CardModel
{
	...
	protected override IEnumerable<DynamicVar> CanonicalVars => new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(new DynamicVar[4]
	{
		...
		new CalculatedVar("CalculatedForge").WithMultiplier(delegate(CardModel card, Creature? target)
		{
			Creature target2 = target;
			CardModel card2 = card;
			return CombatManager.Instance.History.Entries.OfType<DamageReceivedEntry>().Count((DamageReceivedEntry e) => e.Receiver == target2 && e.Dealer == card2.Owner.Creature && e.Result.Props.IsPoweredAttack() && e.HappenedThisTurn(card2.CombatState));
		})
	});
