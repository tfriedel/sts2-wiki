...
public sealed class Spite : CardModel
{
	...
	private static bool LostHpThisTurn(Creature creature)
	{
		...
		return CombatManager.Instance.History.Entries.OfType<DamageReceivedEntry>().Any((DamageReceivedEntry e) => e.HappenedThisTurn(creature2.CombatState) && e.Receiver == creature2 && e.Result.UnblockedDamage > 0);
	}
}
